...
export const addProducts = createAsyncThunk('products/addProducts',async(product, thunkApi) => {
    ...
    try {
        ...
        const { data } = await axios.post(`http://localhost:8000/${type}`,formValue)
...
//  ====================================================================================
export const deleteProducts = createAsyncThunk('products/deleteProducts',async(products, thunkApi) => {
    const {rejectWithValue} =thunkApi
    try {
        let {type , id} = products
        const { data } = await axios.delete(`http://localhost:8000/${type}/${id}`)
...
// ================================================================================
export const editeProducts = createAsyncThunk('products/editeProducts',async(product, thunkApi) => {
    const {rejectWithValue} =thunkApi
    try {
        let {type,id,formValue} = product
        
        const { data } = await axios.put(`http://localhost:8000/${type}/${id}`,formValue)
...
let productsSlice = createSlice({
    ...
    extraReducers: {
        ...
        [addProducts.fulfilled]: (state, action) => {
            console.log( "add api:::::" ,action.payload)
