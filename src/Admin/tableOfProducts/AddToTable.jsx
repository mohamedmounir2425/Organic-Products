...
export default function AddToTable() {
  ...
  const addNewProduct = (e) => {
    ...
    dispatch(addProducts({ type, formValue }))
      .then(navigate(`/products/${type}`))
      .catch((err) => {});
  };
  const editeProduct = (e) => {
    e.preventDefault();

    dispatch(editeProducts({ type, id, formValue }))
      .then(navigate(`/products/${type}`))
      .catch((err) => {});
  };
        ...
        <div className="container my-5">
          <div className="row">
            <h2 className={`  text-muted text-center mb-4`}>
              {" "}
              {id ? (
                <span className="fs-1 fw-bold text-uppercase text-warning">
                  <i className="fa-solid fa-screwdriver-wrench text-dark"></i>{" "}
                  Edite
                </span>
              ) : (
                <span className="fs-1 fw-bold text-uppercase text-success">
                  <i className="fa-solid fa-plus text-dark"></i> Add
                </span>
              )}{" "}
              Product
            </h2>
            <div className={`${style.form} py-3 mx-auto  border rounded`}>
              <form className={` px-3 py-4`}>
                ...
                <div className="col-12">
                  <button
                    onClick={id ? editeProduct : addNewProduct}
                    type="submit"
                    className={`${style.btn} float-end btn mt-2`}
                  >
